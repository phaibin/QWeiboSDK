...
- (NSString *)syncRequestWithUrl:(NSString *)aUrl 
	...
	} else if (!dataList || [dataList count] == 0) {
		retString = [http httpPost:oauthUrl queryString:queryString];
	} else {
		retString = [http httpPostWithData:dataList url:oauthUrl queryString:queryString];
	}
...
- (NSURLConnection *)asyncRequestWithUrl:(NSString *)aUrl 
	...
	if ([aHttpMethod isEqualToString:@"GET"]) {
		...
		connection = [http httpPostWithData:dataList url:oauthUrl queryString:queryString delegate:aDelegate];
	}
